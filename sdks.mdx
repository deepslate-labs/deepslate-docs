...
<CardGroup cols={2}>
  ...
  <Card title="@deepslate/livekit (Node.js)" icon="tower-broadcast" href="/livekit-node">
...
Each language has a shared foundation that its plugins are built on — `deepslate-core` (Python) and `@deepslate/core` (Node.js). It handles WebSocket connectivity, protobuf framing, session lifecycle, and exponential-backoff reconnection.
