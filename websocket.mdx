...
const initMessage = ServiceBoundMessage.create({
  initializeSessionRequest: {
    ...
    supportsPlaybackReporting: true  // This client will report playback positions
  }
});
...
Set `supportsPlaybackReporting: true` when your client will report how many audio bytes it has played. Reporting gives you accurate context truncation when the caller interrupts mid-response. See [Playback Position Reporting](#playback-position-reporting).
...
Set `supportsPlaybackReporting: true` in `InitializeSessionRequest` when you intend to send these.
