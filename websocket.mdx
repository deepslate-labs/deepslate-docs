...
- **`ModelAudioChunk`** — sent only when a **TTS provider is configured**. Contains synthesized audio. The `transcript` field is optionally populated: Deepslate's hosted TTS leaves it empty, while third-party providers such as ElevenLabs may include it for alignment data.
</Info>
...
ws.on('message', (data) => {
  ...
  if (message.modelAudioChunk) {
    ...
    // transcript is optionally populated — hosted TTS leaves it empty;
    // third-party providers may include it for alignment data
...
async function main() {
  ...
  ws.on('message', (data) => {
    ...
    if (msg.modelAudioChunk) {
      // Only received in TTS mode; transcript may be empty depending on provider
