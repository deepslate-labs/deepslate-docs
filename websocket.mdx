...
ws.on('message', (data) => {
  ...
  if (message.responseBegin) {
    ...
    console.log(`Response started (turn ${message.responseBegin.turnId})`);
  }
...
Counts are per turn, not per session. Use the `turnId` from the `ModelAudioChunk` the bytes came from, and keep a separate total for each turn.
...
async function main() {
  ...
  ws.on('message', (data) => {
    ...
    if (msg.responseBegin) {
      console.log(`\n[Response started: turn ${msg.responseBegin.turnId}]`);
    }
