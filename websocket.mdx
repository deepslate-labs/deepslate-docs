...
Set `supportsPlaybackReporting: true` if your client can report how many audio bytes have been played. This enables accurate context truncation when the user interrupts the model mid-response. See [Playback Position Reporting](#playback-position-reporting).
...
ws.on('message', (data) => {
  ...
  if (message.modelAudioChunk) {
    ...
    // transcript is optional and may be absent — hosted TTS omits it;
    // third-party providers (e.g., ElevenLabs) may include it for alignment data
...
async function main() {
  ...
  ws.on('message', (data) => {
    ...
    if (msg.modelAudioChunk) {
      // Only received in TTS mode; transcript is optional and may be absent depending on provider
