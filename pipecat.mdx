...
| Frame | Description |
|-------|-------------|
| `DeepslateExportChatHistoryFrame` | Request a full chat history export. `await_pending: bool` — wait for in-flight ops before exporting. `exclude_audio: bool` — omit audio blobs from the result (transcripts only). |
