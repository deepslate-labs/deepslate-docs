...
Set `uninterruptable=True` to make the utterance play to completion: overlapping user speech is ignored until playback finishes.
...
| `DeepslateDirectSpeechFrame` | Speak text directly via TTS, bypassing the LLM. `text: str`, `include_in_history: bool`, `uninterruptable: bool`. |
