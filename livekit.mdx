---
title: 'LiveKit Plugin (Python)'
description: 'Integrate Deepslate voice AI with LiveKit Agents (Python) for real-time voice applications'
...
import deepslate.livekit
from deepslate.livekit import DeepslateRealtimeSession, ElevenLabsTtsConfig
...
await session.llm.session().speak_direct(
    ...
    uninterruptable=False,    # Allow the user to barge in (default: False)
)
```
...
Setting `uninterruptable=True` makes the utterance play to completion: overlapping user speech is ignored until playback finishes.
