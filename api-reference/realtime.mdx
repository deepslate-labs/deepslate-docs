...
<AccordionGroup>
  ...
  <Accordion title="ModelAudioChunk" icon="volume-high">
    ...
    | `transcript` | string | Optionally populated transcript text accompanying this audio chunk. See note below. |

    <Note>
    The `transcript` field is present on every `ModelAudioChunk` but is **only optionally populated** — do not rely on it being set.

    - **Deepslate hosted TTS** does not populate `transcript`.
    - **Third-party TTS providers** (e.g., ElevenLabs) may optionally include it to supply word-level alignment data alongside the audio.
    </Note>
