...
<AccordionGroup>
  <Accordion title="InitializeSessionRequest" icon="play" defaultOpen>
    **Must be the first message sent.** Configures the session parameters.
    ...
    | `supports_playback_reporting` | bool | Set to `true` when the client intends to send `PlaybackPositionReport` messages. Defaults to `false`. |
  ...
  <Accordion title="PlaybackPositionReport" icon="gauge-high">
    ...
    | `turn_id` | uint32? | Which assistant turn these bytes belong to. Should always be set. |
