...
Call Event Hooks are outbound HTTP webhooks that fire at specific points in a call's lifecycle. When a call starts or ends, the platform sends an HTTP request with a JSON payload to every URL registered on that Assistant or Agent, using the HTTP method configured for that hook (`POST` by default). Use them to trigger workflows, sync conversation data, send transcripts to your CRM, or any other external integration.
...
Webhooks are delivered using the configured HTTP method with `Content-Type: application/json`. Fields whose value is `null` are omitted from the payload.

<Note>
The event payload is always sent as the JSON request body, regardless of the configured method, including for `GET` and `DELETE`. It is never sent as query parameters.
</Note>
...
3. Click **Add Hook** (**Hinzufügen**), select the hook type, and enter your target URL and HTTP method.
